 ...
 */
#if 1
uint64_t *g_lut_intel = nullptr;
#else
// Backed Lookup Table for Intel
...
void init_golden_intel() {
  constexpr uint32_t kN = 256;
  g_lut_intel = (uint64_t *)new uint32_t[kN << 1];
  compute_golden_lut_intel((uint32_t *)g_lut_intel, kN);
}

void deinit_golden_intel() { delete[] g_lut_intel; }

#if 0
void print_golden_lut_intel(uint32_t *pTbl, uint32_t n) {
  printf("static constexpr uint64_t g_lut_intel[] = {\n");
  for (uint32_t i = 0; i < n; ++i) {
    printf("0x%08x%08x,%c", pTbl[i], pTbl[(i << 1) + 1],
           (i & 3) == 3 ? '\n' : ' ');
  }
  printf("};\n");
}
...
#define CRC_ITERS_256_TO_2()                                                   \
  do {                                                                         \
    X0(256)                                                                    \
    X1(254) X2(250) X3(242) X4(226) X5(194) X6(130) X7(2) default : break;     \
  } while (0)

// must be >= 24
static constexpr uint32_t LEAF_SIZE_INTEL = 6 * 24;

// Golden crc32c implementation using Intel hardware instructions
uint32_t crc32c_golden_intel(const void *M, uint32_t bytes,
                             uint32_t prev /* = 0*/) {
  ...
  while (bytes >= LEAF_SIZE_INTEL) {
    ...
    const __m128i vK =
        _mm_cvtepu32_epi64(_mm_loadu_si128((__m128i *)(&g_lut_intel[n - 1])));
